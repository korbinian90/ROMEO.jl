...
# Every form the command line accepts, checked against what the eval this
# replaced used to return. `nargs = '+'` is why the same value arrives split
# across several strings when the user separates it with spaces.
cases = [
    (["[1.5,3.0]"],     [1.5, 3.0]),
    (["[1.5,", "3.0]"], [1.5, 3.0]),
    (["1.5", "3.0"],    [1.5, 3.0]),
    (["[1.5", "3.0]"],  [1.5, 3.0]),   # parsed as a Matrix before, flattened by the caller
    (["1:3"],           [1, 2, 3]),
...
# All-integer input stays integer, because echo selections index with it; one
# float makes the whole thing float, as Julia's own parser did.
@test ROMEO.parse_array(["1:3"]) isa Vector{Int}
@test ROMEO.parse_array(["1,2.5"]) isa Vector{Float64}
@test ROMEO.parse_array(["3"]) isa Int

# The point of the exercise: what the user types is data, not code. Anything
# that is not a number, range or colon is refused with the offending text named.
for bad in (["run(`whoami`)"], ["exit(1)"], ["[1,foo]"], ["1:2:3:4"], [""])
    @test_throws ArgumentError ROMEO.parse_array(bad)
end

# Ranges and the colon are only ever used to index with, which still works.
